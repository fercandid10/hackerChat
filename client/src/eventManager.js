...
export default class EventManager{
    ...
    disconnectUser(user){
        ...
        this.#updateUsersComponent(`${userName} saiu!`)
    }
    ...
    newUserConnected(message){
        const user = message
        this.#allUsers.set(user.id, `${user.userName} entrou!`)
        this.#updateUsersComponent()
    ...
    #updateActivityLogComponent(message){
        this.componentEmitter.emit(
            constants.events.app.ACTIVITYLOG_UPDATED,  message
        )
    }
    ...
    getEvents() {
        const functions = Reflect.ownKeys(EventManager.prototype).filter(fn => fn !== 'constructor').map(name => [name, this[name].bind(this)])
